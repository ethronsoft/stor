...
Status Win32Env::GetChildren(const std::string& dir, std::vector<std::string>* result)
{
	...
	** changing "\\*.*" to
	** "\\*" to prevent files
	** without extensions from being ignored
	** This will in turn return folders as well,
	** which is inline with name "GetChildren"
	**/
    //path += "\\*.*";
	path += "\\*";
	std::wstring wpath;
...
Status Win32Env::CreateDir( const std::string& dirname )
{
	...
	/**
	** commenting out as it makes CreateDirInner
	** attempting to create a final empty directory \\""
	** which results in status being set to IOError even if the
